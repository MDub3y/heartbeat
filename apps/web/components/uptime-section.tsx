...
import { ScrollText, Globe, Clock } from "lucide-react";
import { FadeIn } from "@/components/ui/fade-in"; // Imported
import { SectionHeader } from "@/components/ui/section-header"; // Imported
...
export function UptimeSection() {
  return (
    <section className="mt-32 relative z-10 container mx-auto px-4 sm:px-6" id="uptime-monitoring">
      ...
      {/* --- REFACTORED HEADER --- */}
      <SectionHeader 
        title={<>Best-in-class uptime monitoring. <br className="hidden md:block" /> No false positives.</>}
        description="Get a screenshot of the error and a second-by-second timeline with our fastest 30-second checks."
        ctaText="Explore website monitoring"
        ctaHref="#"
      />
      ...
      <div className="mt-20 mx-auto relative flex flex-col md:flex-row max-w-[900px] gap-8">
        ...
        <FadeIn direction="right" className="flex-1 mt-10 md:mt-0 relative">
          {/* Desktop Image */}
        ...
        <div className="flex-1 flex flex-col gap-6 mt-10 md:mt-20">
          ...
          {/* Card 1: Using the new .card-premium class logic + FadeIn wrapper */}
          <FadeIn delay={0.2} className="relative mt-12 md:mt-4 card-premium border-[#939DB8]/10 bg-[#0F101A]">
            ...
            <div className="relative z-10 px-8 pt-6 pb-8 md:pt-[240px]">
              <h4 className="text-white font-semibold text-lg">Screenshots & error logs</h4>
          ...
          {/* Card 2 */}
          <FadeIn delay={0.3} className="relative px-8 py-6 card-premium min-h-[320px]">
            {/* ... Content ... */}
             <div className="absolute inset-0 w-full h-full">
               <div className="absolute inset-0 bg-gradient-to-b from-transparent via-[#0F101A]/80 to-[#0F101A] z-10" />
               <Image src="/landing/playwright.jpg" fill alt="Playwright Code" className="object-cover object-top opacity-60" />
            </div>
            <div className="relative z-20 mt-[180px]">
              <h4 className="text-white font-semibold text-lg">Playwright transaction monitoring</h4>
              <p className="mt-2 text-[#9CA3AF] text-sm leading-relaxed">
...
function FeatureItem({ icon, title, desc }: { icon: React.ReactNode, title: string, desc: string }) {
  return (
    <FadeIn className="flex-1 max-w-[340px]">
